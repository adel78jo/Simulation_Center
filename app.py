...
def login_user(username, password):
    if username == ADMIN_USER and password == ADMIN_PASS:
        ...
        st.experimental_rerun() # إعادة تشغيل الصفحة لعرض المحتوى
    else:
    ...
    if st.session_state['logged_in']:
        ...
        with login_col1:
            with st.form("login_form"):
                username = st.text_input("اسم المستخدم", placeholder="AABU")
                password = st.text_input("كلمة المرور", type="password", placeholder="Aabu2025")
