...
<style>
    ...
    h1, h2, h3, h4, h5, h6 {
        color: #1a5e58; /* لون أخضر داكن مائل للأزرق */
        font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;
    }
    /* الألوان الأساسية للأزرار والمكونات */
    .stButton>button {
        background-color: #2e8b57; /* أخضر زمردي */
        color: white;
        border: none;
        border-radius: 8px;
        padding: 10px 20px;
    ...
    .stButton>button:hover {
        background-color: #3cb371; /* أخضر فاتح عند المرور */
    }
    /* تصميم بطاقات الـ Metrics */
    [data-testid="stMetric"] {
        background-color: #f0f8ff; /* خلفية فاتحة للبطاقات */
        border-radius: 10px;
        padding: 15px;
    ...
    [data-testid="stMetricLabel"] {
        font-size: 1.1em;
        color: #4CAF50; /* أخضر للعلامة */
        font-weight: bold;
    }
    [data-testid="stMetricValue"] {
        font-size: 2.5em;
        color: #1a5e58; /* أخضر داكن للقيمة */
        font-weight: bolder;
    }
...
st.sidebar.markdown("# 🏛️مركز النمذجة والمحكاة ")
...
st.sidebar.markdown("---")
st.sidebar.info("هذا النظام التجريبي قيد التطوير. البيانات مؤقتة.")
...
if menu == "🏠 الصفحة الرئيسية":
    ...
    **رؤيتنا:** الارتقاء بالكفاءات، ضمان الجودة، وتحفيز الابتكار في مجالات النمذجة والمحاكاة.
    ...
    st.columns(3)[0].markdown("#### 👨‍🏫 التدريب المتخصص")
    st.columns(3)[0].info("ورش عمل ودورات في أحدث تقنيات النمذجة والمحاكاة.")
    st.columns(3)[1].markdown("#### 🔎 ضمان الجودة والتدقيق")
    st.columns(3)[1].info("متابعة دورية لجودة المرافق والبرامج.")
    st.columns(3)[2].markdown("#### 📢 التوعية والبحث")
    st.columns(3)[2].info("نشر المعرفة ودعم المشاريع البحثية المبتكرة.")
...
# ==========================================
    ...
    st.title("🔑 التحكم المركزي ببيانات الشعبة")
    ...
    with tab_courses:
        ...
        with col_c1.expander("➕ إضافة دورة جديدة"):
            with st.form("add_course_admin_form", clear_on_submit=True, key="add_course_form_key"):
        ...
        with col_c2.expander("✍️ تعديل دورة موجودة"):
            if course_ids:
                ...
                with st.form("update_course_admin_form", key="update_course_form_key"):
    ...
    with tab_trainees:
        ...
        with col_t1.expander("✍️ تعديل بيانات متدرب"):
            if trainee_ids and st.session_state['courses']:
                ...
                with st.form("update_trainee_admin_form", key="update_trainee_form_key"):
    ...
    with tab_audit:
        ...
        with col_a1.expander("✍️ تعديل تقرير تدقيق"):
            if audit_ids:
                ...
                with st.form("update_audit_admin_form", key="update_audit_form_key"):
